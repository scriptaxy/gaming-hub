 ...
 1 => 1,
  ...
  public override string TitleForHeader(UITableView tableView, nint section) => section switch
   ...
   3 => "About Synktra",
    ...
    {
   if (section == 3)
 return "© 2025 Scriptaxy. All rights reserved.";
        return "";
     }
...
_profile = await SteamService.Instance.GetPlayerSummaryAsync(_userData.SteamId, _userData.SteamApiKey);
  ...
  if (_profile != null)
     ...
     return section switch { 0 => 2, 1 => 1, _ => 0 };
        ...
        {
    if (_profile != null)
       return section switch { 0 => "Connected Account", 1 => "Steam Credentials", 2 => "Actions", _ => "" };
       return section switch { 0 => "Steam Credentials", 1 => "Actions", _ => "" };
}
...
if (!string.IsNullOrEmpty(_profile.AvatarUrl))
 ...
 cell.SelectionStyle = UITableViewCellSelectionStyle.Default;
    ...
    try
   {
    ...
    if (image != null)
        InvokeOnMainThread(() => { cell.ImageView!.Image = image; cell.ImageView.Layer.CornerRadius = 25; cell.ImageView.ClipsToBounds = true; cell.SetNeedsLayout(); });
      }
        catch { }
      }
...
public override void ViewDidLoad()
  ...
  _profile = await EpicGamesService.Instance.GetAccountInfoAsync(_userData.EpicAccessToken, _userData.EpicAccountId);
      ...
      public override string TitleForHeader(UITableView tableView, nint section) => _profile != null ? (section == 0 ? "Connected Account" : "Actions") : "Connect Epic Games";
        public override string TitleForFooter(UITableView tableView, nint section) => _profile == null && section == 0 ? "Sign in with your Epic Games account to sync your game library." : "";
     ...
     else
  {
   ...
   cell.TextLabel.TextAlignment = UITextAlignment.Center;
    cell.SelectionStyle = _isLoading ? UITableViewCellSelectionStyle.None : UITableViewCellSelectionStyle.Default;
       if (_isLoading) { var spinner = new UIActivityIndicatorView(UIActivityIndicatorViewStyle.Medium); spinner.StartAnimating(); cell.AccessoryView = spinner; }
      }
          else
     { cell.TextLabel!.Text = "Disconnect Epic Games"; cell.TextLabel.TextColor = UIColor.SystemRed; cell.TextLabel.TextAlignment = UITextAlignment.Center; }
    }
   }
        else
            {
...
if (_profile != null && indexPath.Section == 1)
  ...
  if (indexPath.Row == 0) SyncLibrary();
        ...
        else if (_profile == null) StartEpicLogin();
  }
 ...
 private void StartEpicLogin()
       ...
       var nav = new UINavigationController(webVC);
        ...
        private void OnLoginCompleted(object? sender, EpicAuthResult result)
        {
   ...
   await DatabaseService.Instance.SaveUserDataAsync(_userData);
    ...
    private async void SyncLibrary()
   {
   if (string.IsNullOrEmpty(_userData?.EpicAccessToken)) { ShowAlert("Not Connected", "Please connect your Epic Games account first."); return; }
  _isLoading = true; TableView.ReloadData();
        try { var count = await EpicGamesService.Instance.SyncLibraryAsync(_userData.EpicAccessToken); ShowAlert("Sync Complete", $"Successfully synced {count} games from Epic Games!"); }
   catch (Exception ex) { ShowAlert("Sync Failed", ex.Message); }
      finally { _isLoading = false; TableView.ReloadData(); }
  }
    ...
    private void DisconnectEpic()
      ...
      alert.AddAction(UIAlertAction.Create("Cancel", UIAlertActionStyle.Cancel, null));
  alert.AddAction(UIAlertAction.Create("Disconnect", UIAlertActionStyle.Destructive, async _ =>
    { _userData.EpicAccountId = null; _userData.EpicAccessToken = null; _userData.EpicTokenExpiry = null; _userData.EpicLastSync = null; await DatabaseService.Instance.SaveUserDataAsync(_userData); _profile = null; TableView.ReloadData(); }));
   PresentViewController(alert, true, null);
        }
  ...
  private GOGProfile? _profile;
        ...
        public override void ViewDidLoad() { base.ViewDidLoad(); Title = "GOG Galaxy"; TableView = new UITableView(View!.Bounds, UITableViewStyle.InsetGrouped); LoadData(); }
        private async void LoadData() { _userData = await DatabaseService.Instance.GetUserDataAsync(); if (!string.IsNullOrEmpty(_userData.GogAccessToken)) _profile = await GOGService.Instance.GetUserDataAsync(_userData.GogAccessToken); TableView.ReloadData(); }
        public override nint NumberOfSections(UITableView tableView) => _profile != null ? 2 : 1;
      public override nint RowsInSection(UITableView tableView, nint section) => _profile != null ? (section == 0 ? 1 : 2) : 1;
    public override string TitleForHeader(UITableView tableView, nint section) => _profile != null ? (section == 0 ? "Connected Account" : "Actions") : "Connect GOG Galaxy";
        public override string TitleForFooter(UITableView tableView, nint section) => _profile == null && section == 0 ? "Sign in with your GOG account to sync your DRM-free game library." : "";
 ...
 if (_profile != null)
  {
 if (indexPath.Section == 0)
        { cell.TextLabel!.Text = _profile.Username; cell.DetailTextLabel!.Text = _profile.Email ?? _profile.UserId; cell.ImageView!.Image = UIImage.GetSystemImage("person.circle.fill"); cell.ImageView.TintColor = UIColor.FromRGB(134, 50, 179); cell.SelectionStyle = UITableViewCellSelectionStyle.None; }
    else
      {
     if (indexPath.Row == 0)
     { cell.TextLabel!.Text = _isLoading ? "Syncing..." : "Sync Library Now"; cell.TextLabel.TextColor = UIColor.SystemBlue; cell.TextLabel.TextAlignment = UITextAlignment.Center; cell.SelectionStyle = _isLoading ? UITableViewCellSelectionStyle.None : UITableViewCellSelectionStyle.Default; if (_isLoading) { var spinner = new UIActivityIndicatorView(UIActivityIndicatorViewStyle.Medium); spinner.StartAnimating(); cell.AccessoryView = spinner; } }
    else
       { cell.TextLabel!.Text = "Disconnect GOG"; cell.TextLabel.TextColor = UIColor.SystemRed; cell.TextLabel.TextAlignment = UITextAlignment.Center; }
     }
            }
    else
    { cell.TextLabel!.Text = "Sign in with GOG"; cell.TextLabel.TextColor = UIColor.SystemBlue; cell.TextLabel.TextAlignment = UITextAlignment.Center; cell.ImageView!.Image = UIImage.GetSystemImage("arrow.right.circle.fill"); cell.ImageView.TintColor = UIColor.FromRGB(134, 50, 179); }
       return cell;
      }
 ...
 public override void RowSelected(UITableView tableView, NSIndexPath indexPath)
        {
   tableView.DeselectRow(indexPath, true);
  if (_profile != null && indexPath.Section == 1) { if (indexPath.Row == 0) SyncLibrary(); else DisconnectGOG(); }
    else if (_profile == null) StartGOGLogin();
      }

        private void StartGOGLogin() { var webVC = new GOGLoginWebViewController(); webVC.LoginCompleted += OnLoginCompleted; var nav = new UINavigationController(webVC); PresentViewController(nav, true, null); }
        private void OnLoginCompleted(object? sender, GOGAuthResult result) { DismissViewController(true, async () => { _userData.GogAccessToken = result.AccessToken; _userData.GogTokenExpiry = DateTime.UtcNow.AddSeconds(result.ExpiresIn); await DatabaseService.Instance.SaveUserDataAsync(_userData); LoadData(); }); }
  private async void SyncLibrary() { if (string.IsNullOrEmpty(_userData?.GogAccessToken)) { ShowAlert("Not Connected", "Please connect your GOG account first."); return; } _isLoading = true; TableView.ReloadData(); try { var count = await GOGService.Instance.SyncLibraryAsync(_userData.GogAccessToken); ShowAlert("Sync Complete", $"Successfully synced {count} games from GOG!"); } catch (Exception ex) { ShowAlert("Sync Failed", ex.Message); } finally { _isLoading = false; TableView.ReloadData(); } }
      private void DisconnectGOG() { var alert = UIAlertController.Create("Disconnect GOG", "Are you sure you want to disconnect your GOG account?", UIAlertControllerStyle.Alert); alert.AddAction(UIAlertAction.Create("Cancel", UIAlertActionStyle.Cancel, null)); alert.AddAction(UIAlertAction.Create("Disconnect", UIAlertActionStyle.Destructive, async _ => { _userData.GogAccessToken = null; _userData.GogTokenExpiry = null; _userData.GogLastSync = null; await DatabaseService.Instance.SaveUserDataAsync(_userData); _profile = null; TableView.ReloadData(); })); PresentViewController(alert, true, null); }
        private void ShowAlert(string title, string message) { var alert = UIAlertController.Create(title, message, UIAlertControllerStyle.Alert); alert.AddAction(UIAlertAction.Create("OK", UIAlertActionStyle.Default, null)); PresentViewController(alert, true, null); }
    }
 ...
 base.ViewDidLoad();
    ...
    _webView.NavigationDelegate = new GOGWebViewDelegate(this);
     ...
     _loadingIndicator = new UIActivityIndicatorView(UIActivityIndicatorViewStyle.Large) { HidesWhenStopped = true, Center = new CGPoint(View.Bounds.Width / 2, View.Bounds.Height / 2) };
  View.AddSubview(_loadingIndicator);
        var authUrl = GOGService.Instance.GetAuthorizationUrl();
       _webView.LoadRequest(new NSUrlRequest(new NSUrl(authUrl)));
        }

        public override void ViewDidLayoutSubviews() { base.ViewDidLayoutSubviews(); _webView.Frame = View!.Bounds; _loadingIndicator.Center = new CGPoint(View.Bounds.Width / 2, View.Bounds.Height / 2); }
  public async void HandleCallback(string code) { _loadingIndicator.StartAnimating(); var result = await GOGService.Instance.ExchangeCodeForTokenAsync(code); _loadingIndicator.StopAnimating(); if (result != null) LoginCompleted?.Invoke(this, result); else ShowError("Failed to authenticate with GOG"); }
        private void ShowError(string message) { var alert = UIAlertController.Create("Login Failed", message, UIAlertControllerStyle.Alert); alert.AddAction(UIAlertAction.Create("OK", UIAlertActionStyle.Default, _ => DismissViewController(true, null))); PresentViewController(alert, true, null); }

   private class GOGWebViewDelegate : WKNavigationDelegate
        {
     private readonly GOGLoginWebViewController _parent;
            public GOGWebViewDelegate(GOGLoginWebViewController parent) => _parent = parent;
  ...
  if (url?.AbsoluteString?.Contains("on_login_success") == true)
      ...
      var codeItem = components?.QueryItems?.FirstOrDefault(i => i.Name == "code");
            if (codeItem?.Value != null) { _parent.HandleCallback(codeItem.Value); decisionHandler(WKNavigationActionPolicy.Cancel); return; }
        }
       decisionHandler(WKNavigationActionPolicy.Allow);
       }
          public override void DidStartProvisionalNavigation(WKWebView webView, WKNavigation navigation) => _parent._loadingIndicator.StartAnimating();
   public override void DidFinishNavigation(WKWebView webView, WKNavigation navigation) => _parent._loadingIndicator.StopAnimating();
        }
    }
 ...
 public class EpicLoginWebViewController : UIViewController
    {
        private WKWebView _webView = null!;
        private UIActivityIndicatorView _loadingIndicator = null!;
        private const string RedirectUri = "com.scriptaxy.synktra://epic-callback";
 ...
 _webView.NavigationDelegate = new EpicWebViewDelegate(this);
  ...
  var authUrl = EpicGamesService.Instance.GetAuthorizationUrl(RedirectUri);
        ...
        public async void HandleCallback(NSUrl url)
    {
        var components = new NSUrlComponents(url, true);
var codeItem = components?.QueryItems?.FirstOrDefault(i => i.Name == "code");
 if (codeItem?.Value != null) { _loadingIndicator.StartAnimating(); var result = await EpicGamesService.Instance.ExchangeCodeForTokenAsync(codeItem.Value); _loadingIndicator.StopAnimating(); if (result != null) LoginCompleted?.Invoke(this, result); else ShowError("Failed to authenticate with Epic Games"); }
   }
        private void ShowError(string message) { var alert = UIAlertController.Create("Login Failed", message, UIAlertControllerStyle.Alert); alert.AddAction(UIAlertAction.Create("OK", UIAlertActionStyle.Default, _ => DismissViewController(true, null))); PresentViewController(alert, true, null); }
   ...
   private class EpicWebViewDelegate : WKNavigationDelegate
        {
          ...
          public EpicWebViewDelegate(EpicLoginWebViewController parent) => _parent = parent;
       public override void DecidePolicy(WKWebView webView, WKNavigationAction navigationAction, Action<WKNavigationActionPolicy> decisionHandler)
            {
     var url = navigationAction.Request.Url;
     if (url?.Scheme == "com.scriptaxy.synktra") { _parent.HandleCallback(url); decisionHandler(WKNavigationActionPolicy.Cancel); return; }
        decisionHandler(WKNavigationActionPolicy.Allow);
 }
       public override void DidStartProvisionalNavigation(WKWebView webView, WKNavigation navigation) => _parent._loadingIndicator.StartAnimating();
            public override void DidFinishNavigation(WKWebView webView, WKNavigation navigation) => _parent._loadingIndicator.StopAnimating();
