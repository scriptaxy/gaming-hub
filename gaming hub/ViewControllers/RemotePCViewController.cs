...
namespace gaming_hub.ViewControllers
{
 ...
 _streamButton.BackgroundColor = UIColor.FromRGB(124, 58, 237);
...
var padding = 16f;
 ...
 _userData.RemotePCAuthToken);
  ...
  if (_isDiscovering) return;
   ...
   var optionsAlert = UIAlertController.Create(
    "Connect to PC",
       "Choose how to find your PC running Synktra Companion",
    UIAlertControllerStyle.ActionSheet);

            optionsAlert.AddAction(UIAlertAction.Create("Auto-Discover (Scan Network)", UIAlertActionStyle.Default, async _ =>
            {
         await PerformNetworkDiscovery();
    }));

   optionsAlert.AddAction(UIAlertAction.Create("Enter IP Address Manually", UIAlertActionStyle.Default, _ =>
         {
       ShowManualIPEntry();
   }));

            optionsAlert.AddAction(UIAlertAction.Create("Cancel", UIAlertActionStyle.Cancel, _ =>
   {
                _isDiscovering = false;
   }));

   PresentViewController(optionsAlert, true, null);
        }

        private async Task PerformNetworkDiscovery()
    {
            _loadingIndicator.StartAnimating();

      var progressAlert = UIAlertController.Create(
          "Scanning Network...",
         "Looking for PCs running Synktra Companion.\nThis may take 10-15 seconds.",
           UIAlertControllerStyle.Alert);

      progressAlert.AddAction(UIAlertAction.Create("Cancel", UIAlertActionStyle.Cancel, _ =>
            {
        _isDiscovering = false;
 ...
 try
            {
                var discovered = await RemotePCService.Instance.DiscoverPCsOnNetworkAsync(8000);
                await DismissViewControllerAsync(true);

   if (discovered.Count == 0)
      {
        var notFoundAlert = UIAlertController.Create(
      "No PCs Found",
      "Could not find any PCs running Synktra Companion.\n\nTry these:\n• Make sure the companion app is running on your PC\n• Check both devices are on the same WiFi network\n• Try entering the IP address manually",
UIAlertControllerStyle.Alert);

    notFoundAlert.AddAction(UIAlertAction.Create("Enter IP Manually", UIAlertActionStyle.Default, _ =>
    {
      ShowManualIPEntry();
    }));
          notFoundAlert.AddAction(UIAlertAction.Create("OK", UIAlertActionStyle.Cancel, null));
        PresentViewController(notFoundAlert, true, null);
                }
        ...
        alert.AddAction(UIAlertAction.Create("Connect", UIAlertActionStyle.Default, async _ =>
{
  ...
  PresentViewController(alert, true, null);
         ...
         {
           var alert = UIAlertController.Create("Select PC", $"Found {discovered.Count} PCs:", UIAlertControllerStyle.ActionSheet);
...
alert.AddAction(UIAlertAction.Create("Cancel", UIAlertActionStyle.Cancel, null));
    ...
    ShowAlert("Discovery Failed", $"Error: {ex.Message}");
     ...
     private void ShowManualIPEntry()
        {
   var alert = UIAlertController.Create(
             "Enter PC Address",
                "Enter the IP address shown in Synktra Companion on your PC.\n\nFormat: 192.168.x.x",
       UIAlertControllerStyle.Alert);
            ...
            alert.AddTextField(tf =>
  {
     ...
     tf.Text = _userData?.RemotePCHost ?? "";
    });

    alert.AddTextField(tf =>
         {
     tf.Placeholder = "19500";
    ...
    tf.Text = "19500";
            ...
            alert.AddAction(UIAlertAction.Create("Cancel", UIAlertActionStyle.Cancel, _ =>
       {
            ...
            alert.AddAction(UIAlertAction.Create("Connect", UIAlertActionStyle.Default, async _ =>
            {
  var ip = alert.TextFields?[0].Text?.Trim();
      var portText = alert.TextFields?[1].Text?.Trim();
   ...
   _isDiscovering = false;
     ...
     port = 19500;
      ...
      var pc = await RemotePCService.Instance.TryConnectDirectAsync(ip, port);
     ...
     await ConnectToDiscoveredPC(pc);
           ...
           {
                    ShowAlert("Connection Failed", $"Could not connect to {ip}:{port}\n\nMake sure:\n• Synktra Companion is running\n• The IP address is correct\n• Your phone is on the same WiFi network");
           }
